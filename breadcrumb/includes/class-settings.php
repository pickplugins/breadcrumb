<?php

/*
* @Author 		pickplugins
* Copyright: 	2015 pickplugins
*/

if ( ! defined('ABSPATH')) exit;  // if direct access

class class_breadcrumb_settings{

    public function __construct(){

		add_action( 'admin_menu', array( $this, 'breadcrumb_menu_init' ), 12 );

		}


	public function breadcrumb_settings(){
		include('menu/settings.php');	
	}

	
	public	function breadcrumb_menu_init(){

		add_menu_page(__('Breadcrumb', breadcrumb_textdomain), __('Breadcrumb', breadcrumb_textdomain), 'manage_options', 'breadcrumb_settings', array( $this, 'breadcrumb_settings' ), 'dashicons-arrow-right-alt');
		}


	}
	
new class_breadcrumb_settings();